...
&emsp;&emsp;另外还需要注意的地方是，在[1.3.2小节](#singleton_prove)中我们提到了，$b^\*$的值可能并不唯一，所以最终计算出来的$b^\*$可能有多个值，这个时候我们只需要取在所有符合条件的样本点上的**平均值**即可。
&emsp;&emsp;至此，线性分可分SVM的分割超平面参数$\vec w、b$均求解出来了，分割超平面的方程自然就可以得出了。
